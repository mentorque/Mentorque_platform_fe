...
interface MentorUserStatusProps {
  userId: string
  onStatusUpdate?: () => void
}
...
export default function MentorUserStatus({ userId }: MentorUserStatusProps) {
  ...
  const loadStatus = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/mentor/users/${userId}/status`, {
        credentials: 'include',
        headers,
      })
